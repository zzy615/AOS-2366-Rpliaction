...
for n = 1:length(x)
    p_list(n) = x(n);
    [data_index,U_star,S_star,Y] = generate_data(100,2000,3,p_list(n),0.05);      %对于每个sampling rate生成数据
    
    [U_SVD, sigma_SVD, S_SVD] = algorithm1(2000,Y,p_list(n),3);          %输出SVD(algorithm1)的主成分空间及协方差矩阵估计值
    [U_HeteroPCA, sigma_HeteroPCA, S_HeteroPCA] = algorithm2(2000,Y,p_list(n),3,100);  %输出heteroPCA的相应结果
