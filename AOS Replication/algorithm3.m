function [w_square,sigma_U_l,d_l,CRLB,CRUB] = algorithm3(U,sigma,S,Y,p,l,data_index,alpha)
%ALGORITHM3 
% Confidence regions for U_star_{l,.}(1<=l<=d) based on HeteroPCA

% Input:
%   (U,sigma,S):Output of algorithm2
...
d_l = zeros(1,d);
for i = 1:d
    d_l(i) = (w_square(l) + (1-p)*norm(U_sigma(l,:),2)^2)*(w_square(i) + ...
        (1-p)*norm(U_sigma(i,:),2)^2)/(n*p^2) + 2*S(l,i)^2*(1-p)^2/(n * p^2);
end

%compute estimate of sigma_star_{U,l}
UT_U = U(l,:)'*U(l,:); 
sigma_U_l = (((1-p)*norm(U_sigma(l,:),2)^2 + w_square(l))/(n*p))*sigma^(-2) + ...
    2*(1-p)*UT_U/(n*p) + sigma^(-2)*U'*diag(d_l)*U*sigma^(-2);
